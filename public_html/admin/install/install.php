...
// +---------------------------------------------------------------------------+
//
// $Id: install.php,v 1.12 2002/02/21 22:15:58 tony_bibbs Exp $
...
function INST_welcomePage()
{
    ...
    $retval .= '<P>Welcome to Geeklog 1.3.2, the Ultimate Weblog!  Of all the choices of open-source weblogs we are glad you have chosen to install Geeklog.  With Geeklog version 1.3.2 you will be able to experience rich features, easy administration and an extendable platform that is fast and, most importantly, secure!  Ok, enough of the marketing rant...now for the installation! You are only 3 short steps from having Geeklog running on your system.<P>Before we get started it is important that if you are upgrading an existing Geeklog installation you back up your database AND your file system.  This installation script will alter either your Geeklog database, your filesystem or both. Also, if you are upgrading from version 1.3 or older you may need your old lib-database.php file so be sure to save a copy of this file. <b>YOU HAVE BEEN WARNED</b>! <P> Also, this script will only upgrade you from 1.2.5-1 or later to version 1.3.2.  If you are running a version of Geeklog older than 1.2.5-1 then you will need to manaully upgrade to 1.2.5-1 using the scripts in /path/to/geeklog/sql/updates/. This script will do incremental upgrades after this version (i.e. when 1.4 comes out this script will be able to upgrade from 1.2.5-1, 1.3, 1.3.1 or 1.3.2 directly to 1.3.2.  Please note this script will not upgrade any beta version of 1.3. ';
    if ($write_privs) {
...
function INST_getDatabaseSettings($geeklog_path,$upgrade)
{
    ...
    if ($upgrade == 1) {
        ...
        $old_versions = array('1.2.5-1','1.3');
