...
if (INST_phpOutOfDate()) {
    ...
    switch ($step) {
            ...
            if (count($backupFiles) > 0) {
                ...
                foreach ($backupFiles as $filePath) {
                    ...
                    $display .= '<option value="' . $filename . '">' . $backupFile
                        . ' (' . INST_formatSize(@filesize($filePath))
                        . ')</option>' . LB;
                }
                    ...
                    case 'upload':
                        ...
                        if (file_exists($backup_dir . $backupFile['name'])) { // If file already exists.
                            ...
                            if (!move_uploaded_file($backupFile['tmp_name'], $backup_dir . $backup_file_copy)) { // If able to save the file
                                ...
                                $display .= '<p>' . $LANG_MIGRATE[21] . ' <code>' . $backupFile['name'] . '</code> ' . $LANG_MIGRATE[22] . '</p><br' . XHTML . '>' . LB
                                    ...
                                    . '<input type="hidden" name="backup_file" value="' . $backupFile['name'] . '"' . XHTML . '>' . LB
                                    . '<input type="hidden" name="language" value="' . $language . '"' . XHTML . '>' . LB
                            ...
                            if (!move_uploaded_file($backupFile['tmp_name'], $backup_dir . $backupFile['name'])) { // If able to save the uploaded file
                                $display .= $LANG_MIGRATE[19] . $backupFile['name'] . $LANG_MIGRATE[20] . $backup_dir . '.' . LB;
                            } else {
                                header('Location: migrate.php?step=3&dbconfig_path=' . $dbconfig_path
                                    . '&language=' . $language
                                    . '&backup_file=' . urlencode($backupFile['name'])
        ...
        case 3:
            ...
            $unpacked_file = INST_unpackFile($backup_dir, $backupFile, $display);
