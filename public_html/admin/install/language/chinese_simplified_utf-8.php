...
$LANG_HELP = array(
    ...
    7 => '你的数据库用户密码. 若你不肯定，请问你的主机服务员.',
    8 => '有些人想在同一个数据库里安装数个志乐网站. 为了让每一个志乐正确的运行他们必须有独特的表格前缀 (例如： gl1_, gl2_, etc).',
    9 => '肯定这是你的网站的正确 URL, 就是志乐的 <code>index.php</code> 档案存在处 (没有后面的一丿).',
    10 => '有些主机服务者预先设定管理目录 admin. 在这样的情况下，你要将志乐的 admin 目录改名，例如： "myadmin" 而且更改一下的 URL. 现暂时不必改，等到你发现进入 admin菜当是遇到问题.',
    11 => '这是所有志乐寄出的电邮的回信地址和在辛迪加的联络信息.',
    12 => '这是系统电邮的寄信者地址用于用户登记时等等. 这应当是跟网站电邮同样或一个无法回信的地址避免冒名者借着网站登记来录取你的地址. 若这电邮不跟上面一样, 建议你加一个信息在你寄出的电邮里.',
    13 => '指示你是否要用 UTF-8 为你网站的默认字库. 这在多言语的网站上会有帮助.'
);
?>
