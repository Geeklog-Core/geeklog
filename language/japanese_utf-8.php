...
$LANG21 = array(
    ...
    28 => 'PHPブロックのオプション',
    29 => '関数',
    30 => 'PHPコードを使用したブロックを使う場合、phpblock_（例 phpblock_getweather）ではじまるPHPの関数名を上に記入してください。phpblockではじまっていない場合、関数は呼び出されません。これは、悪意を持ったユーザがサイトに有害な関数を定義することを避けるためです。関数の後の（）は不要です。PHPブロックのコードは、/path/to/geeklog/system/lib-custom.phpに記入しておくと、Geeklogをアップグレードしても使用し続けることができます。',
    ...
    38 => 'PHPブロックの関数名は、\'phpblock_\' (例 phpblock_getweather)ではじまっている必要があります。\'phpblock_\'は、セキュリティに配慮した結果ですのでご協力ください。',
    39 => '表示場所',
    40 => '左',
    41 => '右',
